...
sdnofnode(ibn::IBN, node::Int) = ibn.controllers[domain(ibn.cgr, node)]
isintraintent(ibn::IBN, intentt::IntentTree{R}) where {R<:Intent} = ibn.id == src(intentt)[1] == src(intentt)[1]
...
IBN!(counter::Counter, args...) = IBN!(counter(), args...)
"Empty constructor"
IBN(c::Int, ::Type{T}, ::Type{R}) where {T<:SDN, R<:AbstractGraph}  = IBN(c, 
...
function connectIBNs!(ibn1::IBN, ibn2::IBN, cedges::Vector{CompositeEdge{T}}, dprops::Union{Vector{Dict{Symbol,R}}, Nothing}=nothing) where {T, R}
    ...
    CompositeGraphs.add_graph!(ibn1.cgr, (gr2, vcat(vmap2, v2add)), cedges, dprops; both_ways=true)
    #reverse cedges
    CompositeGraphs.add_graph!(ibn2.cgr, (gr1, vcat(vmap1, v1add)), cedges, dprops; both_ways=true, rev_cedges=true)
end
...
function directrealization(ibn::IBN, intimp::ConnectivityIntentCompilation)
    ...
    for e in edgeify(intimp.path)
        if sdnofnode(ibn, e.src) == sdnofnode(ibn, e.dst)
            #intradomain
            succeeded = succeeded && @recargs!(reclist, isavailable(sdnofnode(ibn, e.src), domainedge(ibn.cgr, e), intimp.capacity))
        else
            #interdomain
            succeeded = succeeded && @recargs!(reclist, isavailable(sdnofnode(ibn, e.src), sdnofnode(ibn, e.dst), compositeedge(ibn.cgr, e), intimp.capacity))
        end
        succeeded || break
    end
    if succeeded
...
function withdraw(ibn::IBN, intimp::ConnectivityIntentCompilation)
    for e in edgeify(intimp.path)
        if sdnofnode(ibn, e.src) == sdnofnode(ibn, e.dst)
            #intradomain
            free!(sdnofnode(ibn, e.src), domainedge(ibn.cgr, e), intimp.capacity)
        else
            #interdomain
            free!(sdnofnode(ibn, e.src), sdnofnode(ibn, e.dst), compositeedge(ibn.cgr, e), intimp.capacity)
        end
    end
    return true
end
