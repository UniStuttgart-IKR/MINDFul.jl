@testset ExtendedTestSet "physicaltest.jl"  begin
    ...
    for (reservableresource, lli) in zip([nodeview1, routerview1, oxcview1], [tmlli1, rplli1, oxclli1])
        ...
        RUNJET && @test_opt target_modules=[MINDF] MINDF.canreserve(reservableresource, lli)
    ...
    @test !MINDF.reserve!(oxcview1, MINDF.OXCAddDropBypassSpectrumLLI(1, 4, 0, 6, 5:6), dagnodeid1; checkfirst = true)
    @test MINDF.reserve!(oxcview1, MINDF.OXCAddDropBypassSpectrumLLI(1, 4, 0, 6, 5:6), UUID(2); checkfirst = true)
    ...
    # go from 4 to 1
    @test MINDF.reserve!(MINDF.getoxcview(AG.vertex_attr(ibnag1)[4]), MINDF.OXCAddDropBypassSpectrumLLI(4, 0, 1, 1, 2:4), UUID(3); checkfirst = true)
    @test MINDF.reserve!(MINDF.getoxcview(AG.vertex_attr(ibnag1)[4]), MINDF.OXCAddDropBypassSpectrumLLI(4, 0, 2, 1, 5:6), UUID(4); checkfirst = true)

    # go from 1 to 6
    @test MINDF.reserve!(MINDF.getoxcview(AG.vertex_attr(ibnag1)[6]), MINDF.OXCAddDropBypassSpectrumLLI(6, 1, 1, 0, 2:4), UUID(5); checkfirst = true)
    @test MINDF.reserve!(MINDF.getoxcview(AG.vertex_attr(ibnag1)[6]), MINDF.OXCAddDropBypassSpectrumLLI(6, 1, 2, 0, 5:6), UUID(6); checkfirst = true)
